<div class="row">
    <div class="col-md-6">
        ...
        <div class="form-group">
            <label for="first_name">@lang('Manager')</label>

            <select name="parent_id" id="cars" class="form-control input-solid" {{ $profile ? 'disabled' : '' }}>
                <option value="0">None</option>
                @foreach($parents as $item)
                    <option {{ $edit ? ($item->id==$user->parent_id ? "selected" : "") : "" }} value="{{ $item->id }}" >{{ $item->username }}</option>
                @endforeach
            </select>
        </div>
